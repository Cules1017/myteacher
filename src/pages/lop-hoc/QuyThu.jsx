        ...
        <div className={`absolute top-full mt-2 w-max min-w-full bg-surface-hover border border-border rounded-xl shadow-2xl overflow-hidden z-[60] ${align === "right" ? "right-0" : "left-0"}`}>
          {suggestions.map(s => (
            <button
              key={s}
              type="button"
              className={`w-full ${align === "right" ? "text-right" : "text-left"} px-4 py-2 text-sm font-medium text-primary-600 dark:text-primary-400 hover:bg-surface transition-colors border-b border-border last:border-0`}
              onPointerDown={(e) => e.preventDefault()} // Prevents blur before click
              onClick={() => {
                onChange(String(s));
                setShow(false);
              }}
